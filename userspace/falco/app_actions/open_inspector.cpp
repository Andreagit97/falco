		...
		const std::string& source)
{
	try
	{
		...
		{
			falco_logger::log(LOG_INFO, "Opening capture with modern BPF probe. Buffer mode: " + std::string(get_modern_bpf_buffer_mode_name(m_state->syscall_buffer_mode)));
			inspector->open_modern_bpf(m_state->syscall_buffer_bytes_size, m_state->syscall_buffer_mode, m_state->ppm_sc_of_interest, m_state->tp_of_interest);
		}
